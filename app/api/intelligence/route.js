...
export async function buildIntelligencePayload() {
  ...
  // ── Oil scoring: price-level (GCC budget capacity) + supply-disruption modifier ──
  // Supply disruptions cause oil AND gold to rise simultaneously (safe-haven bid) while VIX
  // elevates. Demand-driven oil rises are accompanied by calm markets (gold flat, low VIX).
  const oilBaseScore  = brent>=88?5 : brent>=78?4 : brent>=68?3 : brent>=58?2 : brent>0?1 : 3;
  const oilPct        = parseFloat((markets.brent?.pct||'0%').replace('%',''));
  const goldPct       = parseFloat((markets.gold?.pct||'0%').replace('%',''));
  // Both oil and gold rising together signals a geopolitical/supply-shock rather than demand growth
  const oilAndGoldRising = oilPct > 1.0 && goldPct > 0.5;
  const vixElevated      = vix >= 22;
  const confirmedDisrupt = oilAndGoldRising && vixElevated; // strong multi-signal: -2
  const mildDisruption   = oilAndGoldRising && !vixElevated; // oil+gold only: -1
  const disruptionPenalty = confirmedDisrupt ? 2 : mildDisruption ? 1 : 0;
  const oilScore = Math.max(1, oilBaseScore - disruptionPenalty);
  // Flag used in pillar text and alert_indicators
  const oilDisruptionFlag = confirmedDisrupt ? 'supply_shock' : mildDisruption ? 'possible_disruption' : 'demand_driven';
  ...
  return {
    ...
    alert_indicators: {
      ...
      oil_supply_shock: {
        value: markets.brent?.price,
        alert_level: oilDisruptionFlag!=='demand_driven' ? 'WATCH L1' : null,
        flag: oilDisruptionFlag,
        note: oilDisruptionFlag==='supply_shock'
          ? 'Oil rise appears supply-driven (oil+gold+VIX co-move) — not a GCC wealth signal'
          : oilDisruptionFlag==='possible_disruption'
          ? 'Oil and gold rising together — possible supply disruption, monitor closely'
          : null,
      },
    },
    pillars: {
      ...
      oil: {
        ...
        headline: brent>0
          ? `Brent $${markets.brent?.price}/bbl — ${
              ...
              oilDisruptionFlag==='possible_disruption' ? 'possible supply disruption — oil & gold rising together' :
              oilScore>=4 ? 'GCC budgets balanced, capital flowing' : 'pressure on Gulf sovereign wealth'
            }`
          : 'Oil data unavailable',
        bullets: [
          `Brent crude: $${markets.brent?.price||'N/A'} (${markets.brent?.pct||'—'})`,
          `Gold: $${markets.gold?.price||'N/A'} (${markets.gold?.pct||'—'}) — ${
            oilAndGoldRising ? 'rising in tandem with oil = safe-haven bid, supply-shock pattern' : 'moving independently of oil = demand-driven signal'
          }`,
          oilDisruptionFlag==='supply_shock'
            ? `VIX ${markets.vix?.price||'—'} elevated — confirmed supply disruption: GCC budget benefit lags price surge`
            : oilDisruptionFlag==='possible_disruption'
            ? `VIX ${markets.vix?.price||'—'} moderate — oil+gold co-move suggests possible supply pressure; monitor`
            : oilScore>=4
            ? 'GCC sovereign budgets in surplus — Dubai property allocations intact'
            : 'Monitor GCC buyer activity at DLD',
        ],
        risk: oilDisruptionFlag!=='demand_driven'
          ? 'Supply-driven oil spike does not translate to GCC buyer activity — GCC spending lags price by weeks; global instability may suppress transaction volumes'
          : 'Sustained sub-$65 Brent compresses GCC sovereign wealth flows into Dubai',
        action: brent>0&&brent<70
          ? 'Low oil — monitor Gulf buyer volumes at DLD closely.'
          : oilDisruptionFlag==='supply_shock'
          ? 'Supply shock flagged — do not read high oil as bullish. Monitor GCC transaction volumes and global risk tone before assuming demand uplift.'
          : oilDisruptionFlag==='possible_disruption'
