...
export function mergeRentalIntoPayload(payload, csvRaw, rentalLabel, windows, opts = {}) {
  ...
  if (!recordsFiltered.length) {
    ...
    payload.rental.note = filterBuilding
      ? `Rental CSV: no rows for building “${filterBuilding}”${filterArea ? ` in “${filterArea}”` : ''}. Clear building filter or check column names.`
      : filterArea
  ...
  const source = filterBuilding
    ? `Rental CSV (${rentalLabel}) · ${filterBuilding}${filterArea ? ` (${filterArea})` : ''}`
    : filterArea
