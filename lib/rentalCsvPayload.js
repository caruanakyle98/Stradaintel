...
export function mergeRentalIntoPayload(payload, csvRaw, rentalLabel, windows, opts = {}) {
  ...
  /** @type {{ key: string, txnCount: number, avgRounded: number|null, passesMinTxn: boolean }[]} */
  const sunriseBenchDebug = [];
  for (const [key, agg] of txnByBuildingBedAgg.entries()) {
    if (key.toLowerCase().includes('sunrise')) {
      const avgR = agg.count > 0 ? Math.round(agg.sum / agg.count) : null;
      sunriseBenchDebug.push({
        key,
        txnCount: agg.count,
        avgRounded: avgR,
        passesMinTxn: agg.count >= hotMinTxn,
      });
    }
  ...
  if (typeof fetch !== 'undefined') {
    const sr = recordsFiltered.filter((r) => {
      const t = `${r.subCommunity || ''} ${r.communityBuilding || ''}`.toLowerCase();
      return t.includes('sunrise');
    });
    const sr1 = sr.filter((r) => normalizeBedKey(r.beds) === '1');
    fetch('http://127.0.0.1:7603/ingest/99cc14af-5ec3-4b0c-b7f2-77017c17c844', {
      ...
      body: JSON.stringify({
        ...
        location: 'rentalCsvPayload.js:txn_by_building_bed',
        message: 'rental hot benchmarks + Sunrise sample',
        data: {
          filterArea: filterArea || null,
          hotLookbackDays,
          hotMinTxn,
          sunriseRowsInFilteredArea: sr.length,
          sunrise1BedRowsInFilteredArea: sr1.length,
          sunrise1BedSample: sr1.slice(0, 8).map((r) => ({
            subCommunity: r.subCommunity,
            communityBuilding: r.communityBuilding,
            rowArea: r.rowArea,
            bedsRaw: r.beds,
            bedKey: normalizeBedKey(r.beds),
            unitTypeKey: r.unitTypeKey,
            inLookback: !!(r.d && r.d >= hotCutoff),
          })),
          sunriseBucketsPreFinal: sunriseBenchDebug,
          sunriseKeysInPayload: Object.keys(txn_by_building_bed).filter((k) => k.toLowerCase().includes('sunrise')),
          harbourCommunityKeys: Object.keys(txn_by_community_bed).filter((k) => k.includes('harbour')),
        },
        timestamp: Date.now(),
      }),
    }).catch(() => {});
