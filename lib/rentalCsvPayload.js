...
export function mergeRentalIntoPayload(payload, csvRaw, rentalLabel, windows) {
  ...
  const rentCol = pickColumn(headers, ['Annual Rent (AED)', 'Rent (AED)', 'Annual Rent', 'Rent', 'Price (AED)', 'Yearly Rent', 'Rent Amount']);
  const bedsCol = pickColumn(headers, ['Beds', 'Bedrooms', 'Bed']);

  if (!dateCol || !rentCol) return payload;

  const records = rows
    .map(r => ({
      d: parseDubaiEvidenceDate(getWithAliases(r, [dateCol])),
