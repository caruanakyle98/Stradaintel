...
function buildMonthlyRental(records) {
  ...
  return {
    ...
    cur_psf_median: curPsf,
    prev_psf_median: prevPsf,
...
export function mergeRentalIntoPayload(payload, csvRaw, rentalLabel, windows, opts = {}) {
  ...
  // Building filter: if communityFilter is set, match Level 3 only (sub-building)
  //                  if communityFilter is NOT set, match Level 2 || Level 3 (existing behaviour)
  if (filterBuilding) {
    recordsFiltered = recordsFiltered.filter((r) => {
      const bName = filterCommunity
        ? (r.subCommunity || '').trim()                              // Level 3 only
        : (r.communityBuilding || r.subCommunity || '').trim();     // existing
