...
export function mergeRentalIntoPayload(payload, csvRaw, rentalLabel, windows, opts = {}) {
  ...
  if (filterCommunity) {
    recordsFiltered = recordsFiltered.filter((r) =>
      communitiesMatch(r.communityBuilding || r.subCommunity, filterCommunity, null)
    );
  }
  // Building filter.
  // When filterCommunity is NOT set: match Level 2 || Level 3 (communityBuilding || subCommunity).
  // When filterCommunity IS set: DLD exports vary — tower name may be in Level 2 or Level 3
  //   depending on the area. Check both columns so the filter works regardless.
  if (filterBuilding) {
    recordsFiltered = recordsFiltered.filter((r) => {
      if (filterCommunity) {
        const sub   = (r.subCommunity    || '').trim();
        const drill = (r.communityBuilding || '').trim();
        return (sub   && communitiesMatch(sub,   filterBuilding, null)) ||
               (drill && communitiesMatch(drill, filterBuilding, null));
      }
      const bName = (r.communityBuilding || r.subCommunity || '').trim();
