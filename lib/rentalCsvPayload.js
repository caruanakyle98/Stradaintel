...
export function mergeRentalIntoPayload(payload, csvRaw, rentalLabel, windows, opts = {}) {
  ...
  if (filterBuilding) {
    recordsFiltered = recordsFiltered.filter((r) => {
      ...
      const bName      = (r.normBuilding || r.normDistrict || '').trim();
      if (parentName && communitiesMatch(parentName, filterBuilding, null)) return true;
      return bName && communitiesMatch(bName, filterBuilding, null);
    });
  }
