...
export function mergeRentalIntoPayload(payload, csvRaw, rentalLabel, windows, opts = {}) {
  ...
  const aptPrice = payload.prices?.apt_avg_aed_yield_ref_num ?? payload.prices?.apt_avg_aed_num;
  const villaPrice = payload.prices?.villa_avg_aed_yield_ref_num ?? payload.prices?.villa_avg_aed_num;
  const townhousePrice = payload.prices?.townhouse_avg_aed_yield_ref_num ?? payload.prices?.townhouse_avg_aed_num;
