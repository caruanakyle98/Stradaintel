...
function buildRentalTopAreas(records, weekStart, weekEnd) {
  ...
  for (const r of week) {
    const area = r.rowArea || 'Unknown';
    if (!byArea.has(area)) byArea.set(area, { count: 0, rentSum: 0 });
...
export function mergeRentalIntoPayload(payload, csvRaw, rentalLabel, windows, opts = {}) {
  ...
  payload.rental_charts_30d = buildRentalCharts30d(records);
  payload.rental_top_areas = buildRentalTopAreas(records, weekStart, weekEnd);
