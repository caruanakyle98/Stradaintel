...
function buildRentalCharts30d(records) {
  ...
  const psf_weekly = [];
  const daysInWeeks = 5; // 30 days = ~4.3 weeks, but compute weeks in the 30-day window
  for (let weekIdx = 0; weekIdx < 5; weekIdx++) {
    const weekStart = Math.min(weekIdx * 7, 23);
    const weekEnd = Math.min((weekIdx + 1) * 7, 29);
    if (weekStart >= 30) break;
...
export function mergeRentalIntoPayload(payload, csvRaw, rentalLabel, windows, opts = {}) {
  ...
  payload.rental_owner_briefing = filterBuilding
    ? `Building filter "${filterBuilding}"${filterArea ? ` in "${filterArea}"` : ''}. Rolling 7 days (${period}, Dubai): ${weekCount.toLocaleString('en-US')} rental registrations, annualised value AED ${fmtCompact(weekValue)}. Prior 7 days: ${prevCount.toLocaleString('en-US')} registrations.${newPctStr}`
    : filterArea
