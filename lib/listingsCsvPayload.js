...
export function buildListingsPayload(csvRaw, label = 'listings.csv', opts = {}) {
  ...
  try {
    forEachCsvObject(
      ...
      if (filterCommunity && communityCol) {
        const comm = getVal(row, communityCol);
        if (!communitiesMatch(comm, filterCommunity, null)) {
          return;
        }
      }

      if (filterBuilding && buildingCol) {
        const bldg = getVal(row, buildingCol);
        if (!communitiesMatch(bldg, filterBuilding, null)) {
          return;
        }
      }
