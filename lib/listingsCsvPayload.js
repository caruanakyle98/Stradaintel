...
/**
 * Strip a trailing tower/block/phase suffix to get the stem that several related
 * listings share. Returns { stem, suffixKind } where suffixKind is 'tower' |
 * 'residence' | 'block' | 'phase' | 'bare' | null.
 *
 * Examples:
 *   "Beach Isle Tower A"   → stem "Beach Isle",       suffixKind "tower"
 *   "Marina Gate 1"        → stem "Marina Gate",      suffixKind "bare"
 *   "Damac Bay Tower 2"    → stem "Damac Bay",        suffixKind "tower"
 *   "ADDRESS JBR"          → stem "ADDRESS JBR",      suffixKind null
 *
 * Roman numerals are matched up to IV (avoids clipping brand names like "LIV").
 */
function stripTrailingTowerSuffix(name) {
  const s = String(name || '').trim();
  if (!s) return { stem: '', suffixKind: null };

  const labeled = s.match(/^(.*?)\s+(Tower|Towers|Residence|Residences|Block|Phase)\s+(?:[A-Z]|[IVX]{1,4}|\d{1,3})$/i);
  if (labeled) {
    const kind = labeled[2].toLowerCase().replace(/s$/, '');
    return { stem: labeled[1].trim(), suffixKind: kind };
  }

  const bare = s.match(/^(.*?)\s+(?:[A-Z]|[IVX]{1,4}|\d{1,3})$/);
  if (bare && bare[1].trim().length >= 3) {
    return { stem: bare[1].trim(), suffixKind: 'bare' };
  }

  return { stem: s, suffixKind: null };
}

/**
 * Derive listings building/community options from area-filtered rows.
 * Groups buildings by a shared stem (see stripTrailingTowerSuffix). A group is
 * emitted only when 2+ distinct sub-buildings share the stem; otherwise each
 * building stands alone.
 *
 * @param {Array<{ building: string, community: string }>} areaRows
 * @param {string} filterArea
 * @returns {{
 *   buildingOptions: Array<{ name: string, area: string, count: number, sub_buildings: string[] }>,
 *   communityOptions: Array<{ name: string, count: number }>,
 *   parentToChildren: Map<string, Set<string>>,
 * }}
 */
function deriveListingsOptions(areaRows, filterArea) {
  /** @type {Map<string, { display: string, suffixKind: string|null, counts: Map<string, number> }>} */
  const stems = new Map();
  /** @type {Map<string, number>} */
  const commCounts = new Map();

  for (const r of areaRows) {
    const bldg = (r.building || '').trim();
    const comm = (r.community || '').trim();
    if (comm) commCounts.set(comm, (commCounts.get(comm) || 0) + 1);
    if (!bldg) continue;

    const { stem, suffixKind } = stripTrailingTowerSuffix(bldg);
    const key = normalizeCommunityKey(stem);
    if (!stems.has(key)) {
      stems.set(key, { display: stem, suffixKind, counts: new Map() });
    }
    const entry = stems.get(key);
    entry.counts.set(bldg, (entry.counts.get(bldg) || 0) + 1);
    // Prefer a labeled suffix kind (Tower / Residence / Block / Phase) over 'bare'
    // when the same stem appears with both forms.
    if (suffixKind && suffixKind !== 'bare') entry.suffixKind = suffixKind;
  }
  ...
  for (const [key, entry] of stems) {
    const distinctBuildings = [...entry.counts.keys()];
    const totalCount = [...entry.counts.values()].reduce((a, b) => a + b, 0);

    if (distinctBuildings.length >= 2 && entry.suffixKind) {
      // Emit a parent group.
      let display = entry.display;
      if (entry.suffixKind === 'tower')     display += ' Towers';
      else if (entry.suffixKind === 'residence') display += ' Residences';
      buildingOptions.push({
        name: display,
        area: filterArea || '',
        count: totalCount,
        sub_buildings: distinctBuildings.sort(),
      });
      parentToChildren.set(
        normalizeCommunityKey(display),
        new Set(distinctBuildings.map(normalizeCommunityKey)),
      );
    } else {
      // Emit each building individually.
      for (const [b, c] of entry.counts) {
        buildingOptions.push({
          name: b,
          area: filterArea || '',
          count: c,
  ...
  buildingOptions.sort((a, b) => b.count - a.count || a.name.localeCompare(b.name));

  const communityOptions = [...commCounts.entries()]
    .map(([name, count]) => ({ name, count }))
    .sort((a, b) => b.count - a.count || a.name.localeCompare(b.name));

  return { buildingOptions, communityOptions, parentToChildren };
}
...
export function buildListingsPayload(csvRaw, label = 'listings.csv', opts = {}) {
  ...
  /**
   * Pass 1 intermediate rows: already area-filtered, price-parsed, fully parsed.
   * We hold these in memory so Pass 2 can derive parent-building groups from
   * the full area scope before applying the community/building narrow filter.
   * @type {Array<{ price: number, bedKey: string, unitTypeKey: string, listedDate: Date|null, community: string, building: string, link: string|null, rawPriceFailed?: boolean }>}
   */
  const areaRows = [];
  ...
  // Derive listings-scoped building/community options from the full area scope.
  // These power the listings-only secondary filter dropdowns in the UI.
  const { buildingOptions: listings_building_options, communityOptions: listings_community_options, parentToChildren } =
    deriveListingsOptions(areaRows, filterArea);

  // Pass 2: narrow to community/building (parent-aware) and aggregate.
  const fcNorm = normalizeCommunityKey(filterCommunity);
  const fbNorm = normalizeCommunityKey(filterBuilding);
  const fbChildren = filterBuilding ? parentToChildren.get(fbNorm) : null;

  for (const r of areaRows) {
    if (filterCommunity) {
      // Listings community filter matches either the listing community column
      // or (defensively) the building column — some listing CSVs put what
      // users think of as the "community" into the building field.
      const commNorm = normalizeCommunityKey(r.community);
      const bldgNorm = normalizeCommunityKey(r.building);
      if (commNorm !== fcNorm && bldgNorm !== fcNorm) continue;
    }

    if (filterBuilding) {
      const bldgNorm = normalizeCommunityKey(r.building);
      const match = bldgNorm === fbNorm || (fbChildren && fbChildren.has(bldgNorm));
      if (!match) continue;
    }
    ...
    if (r.listedDate) {
      const t = r.listedDate.getTime();
      if (t >= weekStart.getTime() && t <= weekEnd.getTime()) new_this_week++;
      else if (t >= prevStart.getTime() && t <= prevEnd.getTime()) new_prev_7_days++;
    }

    if (!byBedsAgg[r.bedKey]) {
      byBedsAgg[r.bedKey] = { count: 0, sum: 0, min: Infinity, max: -Infinity };
    }
    const agg = byBedsAgg[r.bedKey];
    agg.count++;
    agg.sum += r.price;
    agg.min = Math.min(agg.min, r.price);
    agg.max = Math.max(agg.max, r.price);

    if (r.community) communityCounts[r.community] = (communityCounts[r.community] || 0) + 1;
    if (r.building)  buildingCounts[r.building]   = (buildingCounts[r.building]   || 0) + 1;
    if (r.emirate)   emirateCounts[r.emirate]     = (emirateCounts[r.emirate]     || 0) + 1;
  ...
  // When the listings-only filter narrows to zero rows but the area scope had
  // data, return an OK payload with an empty stats block so the UI can keep the
  // dropdowns visible and let the user clear the filter.
  const listingsFilterActive = !!(filterCommunity || filterBuilding);
  if (total === 0 && listingsFilterActive && areaRows.length > 0) {
    const filterDesc = [filterCommunity, filterBuilding].filter(Boolean).join(' / ');
    return {
      ok: true,
      listings: {
        total: 0,
        data_type: opts.dataType || 'rental',
        new_this_week: dateCol ? 0 : null,
        new_prev_7_days: dateCol ? 0 : null,
        wow_new_pct: null,
        listings_added_by_day: [],
        listings_added_prev_by_day: [],
        listings_added_period: null,
        listings_added_period_prior: null,
        listings_flow: null,
        by_beds: {},
        top_communities: [],
        top_buildings: [],
        ...
        filter_community: filterCommunity || null,
        filter_building: filterBuilding || null,
        listings_top_mode: filterArea ? 'building' : 'community',
        hot_listings: [],
        hot_listings_by_type: { apartment: [], villa: [], townhouse: [] },
        hot_listings_note: `No listings match ${filterDesc} in ${filterArea || 'this area'}.`,
        hot_listings_rules: '',
        listings_building_options,
        listings_community_options,
        columns_found: {
  ...
  if (dateCol) {
    // weekStart/weekEnd/prevStart/prevEnd from deriveAnalysisWindows are already
    // Dubai wall-clock values — format them without a second Asia/Dubai shift.
    // Only r.listedDate (a real instant) needs the Asia/Dubai conversion.
    const daySlots = [];
    for (let i = 0; i < 7; i++) {
