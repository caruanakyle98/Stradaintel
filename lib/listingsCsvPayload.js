...
export function computeHotListings(rows, thirtyDaysAgo, dateColPresent, txnBenchmarkByBuildingBed = {}, dataType = 'rental') {
  ...
  if (recent.length === 0 && rows.length > 0) {
    ...
    hot_listings_note =
      'No listings in the last 30 days are below the transacted average for their building + bedroom + property type (or benchmark sample is too small).';
  }
...
export function buildListingsPayload(csvRaw, label = 'listings.csv', opts = {}) {
  ...
  const hot_listings_rules =
    dataType === 'sales'
      ? 'Top 25 asks below average transacted sale price per building + bedroom + property type (apartment / villa / townhouse) within the selected area — sales CSV, last 30 days of listings.'
      : 'Top 25 asks below average transacted rent per building + bedroom + property type (apartment / villa / townhouse) within the selected area — rental CSV, last 30 days of listings.';
