...
/**
 * Hot Listings: ≤30d listings where asking is below **transacted** rental average
 * for that specific building+bedroom bucket.
 *
 ...
 * @param {Record<string, { avg: number, n: number }>} [rentalTxnByBuildingBed]
 * @returns {{ hot_listings: object[], hot_listings_note: string|null }}
 */
export function computeHotListings(rows, thirtyDaysAgo, dateColPresent, rentalTxnByBuildingBed = {}) {
  if (!dateColPresent) {
    return {
      hot_listings: [],
  ...
  const hasAnyTxn = Object.keys(rentalTxnByBuildingBed || {}).length > 0;

  /** @type {Array<{ community: string, building: string, beds: string, price_fmt: string, pct_drop: number, link: string|null, bed_label: string, market_avg_fmt: string, benchmark_n: number, benchmark_source: string }>} */
  const scored = [];

  for (const r of recent) {
    if (!r.building) continue;
    const bKey = `${normalizeCommunityKey(r.building)}|${r.bedKey}`;
    const bench = rentalTxnByBuildingBed[bKey];
  ...
  if (recent.length === 0 && rows.length > 0) {
    ...
    hot_listings_note =
      'Hot Listings need building-level transacted rental benchmarks (rental CSV). Ensure PROPERTY_RENTAL_CSV_URL is set and building names align.';
  } else if (recent.length > 0 && hot_listings.length === 0 && hasAnyTxn) {
    hot_listings_note =
      'No listings in the last 30 days are below the transacted average for their specific building + bedroom bucket (or benchmark sample is too small).';
  }
...
export function buildListingsPayload(csvRaw, label = 'listings.csv', opts = {}) {
  ...
  const { rentalTxnAvgByBeds, rentalAvgByBeds = {}, rentalTxnByBuildingBed = {}, dataType = 'rental' } = opts;
  ...
  const hot_listings_rules =
    'Top 25 listing asks below transacted rental average for the same building + bedroom count (rental CSV), listed in the last 30 days. Area filter applies.';
