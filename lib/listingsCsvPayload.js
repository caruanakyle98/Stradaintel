...
/**
 * Leave-one-out peer average among ≤30d listings per bed bucket; top under-market % .
 ...
 */
export function computeHotListings(rows, thirtyDaysAgo, dateColPresent) {
  if (!dateColPresent) {
    return {
      hot_listings: [],
  ...
  /** @type {Map<string, { sum: number, items: typeof recent }>} */
  const byBed = new Map();
  for (const r of recent) {
    if (!byBed.has(r.bedKey)) byBed.set(r.bedKey, { sum: 0, items: [] });
    const g = byBed.get(r.bedKey);
    g.sum += r.price;
    g.items.push(r);
  }

  /** @type {Array<{ community: string, building: string, price_fmt: string, pct_drop: number, link: string|null, bed_label: string, market_avg_fmt: string }>} */
  const scored = [];

  for (const r of recent) {
    const g = byBed.get(r.bedKey);
    if (!g || g.items.length < 2) continue;
    const peerAvg = (g.sum - r.price) / (g.items.length - 1);
    if (!(peerAvg > 0) || r.price >= peerAvg) continue;
    const pctDrop = ((peerAvg - r.price) / peerAvg) * 100;
    ...
    scored.push({
      ...
      market_avg_fmt: fmtAed(Math.round(peerAvg)),
    });
  }
...
export function buildListingsPayload(csvRaw, label = 'listings.csv', opts = {}) {
  ...
  const { hot_listings, hot_listings_note } = computeHotListings(listingRows, thirtyDaysAgo, !!dateCol);

  const hot_listings_rules =
    'Top 25 by % below leave-one-out peer average asking (same bedroom count), listed in the last 30 days. Respects area filter.';
