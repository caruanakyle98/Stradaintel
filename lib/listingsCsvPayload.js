...
export function computeHotListings(
  ...
  if (typeof fetch !== 'undefined') {
    const listingSunrise = [];
    for (const r of recent) {
      if (!r.building || !String(r.building).toLowerCase().includes('sunrise')) continue;
      const uk = r.unitTypeKey === 'villa' || r.unitTypeKey === 'townhouse' ? r.unitTypeKey : 'apt';
      const bKey = `${normalizeCommunityKey(r.building)}|${r.bedKey}|${uk}`;
      let effBench = txnBenchmarkByBuildingBed[bKey];
      let effSource = 'building_txn';
      if (!effBench || !(effBench.avg > 0) || !(effBench.n > 0)) {
        const commRaw = (r.community || '').trim() || (filterArea || '').trim();
        if (commRaw) {
          const cKey = `${normalizeCommunityKey(commRaw)}|${r.bedKey}|${uk}`;
          const cB = txnBenchmarkByCommunityBed[cKey];
          if (cB && cB.avg > 0 && cB.n > 0) {
            effBench = cB;
            effSource = 'community_txn';
          }
        }
      }
      listingSunrise.push({
        buildingRaw: r.building,
        bedKey: r.bedKey,
        unitTypeKey: uk,
        lookupKeyBuilding: bKey,
        lookupKeyCommunity: `${normalizeCommunityKey((r.community || '').trim() || (filterArea || '').trim())}|${r.bedKey}|${uk}`,
        askingAnnual: r.price,
        benchHit: !!(effBench && effBench.avg > 0 && effBench.n > 0),
        benchAvg: effBench?.avg ?? null,
        benchN: effBench?.n ?? null,
        benchSource: effSource,
        pctBelowIfEligible:
          effBench && effBench.avg > 0 && r.price < effBench.avg
            ? Math.round(((effBench.avg - r.price) / effBench.avg) * 1000) / 10
            : null,
      });
    }
    const hottestSunrise = scored.find((x) => String(x.building).toLowerCase().includes('sunrise')) || null;
    fetch('http://127.0.0.1:7603/ingest/99cc14af-5ec3-4b0c-b7f2-77017c17c844', {
      ...
      body: JSON.stringify({
        ...
        location: 'listingsCsvPayload.js:computeHotListings',
        message: 'listing Sunrise rows vs benchmark map',
        data: {
          dataType,
          recentSunriseListingRows: listingSunrise,
          hottestScoredSunrise: hottestSunrise,
          totalHotListings: hot_listings.length,
        },
        timestamp: Date.now(),
      }),
    }).catch(() => {});
  ...
  if (total === 0) {
    ...
    if (typeof fetch !== 'undefined') {
      fetch('http://127.0.0.1:7603/ingest/99cc14af-5ec3-4b0c-b7f2-77017c17c844', {
        ...
        body: JSON.stringify({
          sessionId: '69d0ba',
          hypothesisId: 'A-area-filter-B-price-parse-C-wrong-col-D-empty',
          location: 'listingsCsvPayload.js:buildListingsPayload',
          message: 'listings total===0 diagnostics',
          data: {
            priceCol,
            communityCol,
