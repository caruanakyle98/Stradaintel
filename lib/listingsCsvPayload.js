 ...
 */
export function computeHotListings(rows, thirtyDaysAgo, dateColPresent, txnBenchmarkByBuildingBed = {}, dataType = 'rental') {
  if (!dateColPresent) {
    return {
      hot_listings: [],
  ...
  const hasAnyBench = Object.keys(txnBenchmarkByBuildingBed || {}).length > 0;
  ...
  for (const r of recent) {
    ...
    const bench = txnBenchmarkByBuildingBed[bKey];
  ...
  if (recent.length === 0 && rows.length > 0) {
    ...
    hot_listings_note =
      dataType === 'sales'
        ? 'Hot Listings need building-level transacted sale benchmarks (sales transactions CSV). Add a Bedrooms column to sales data and align building/tower names with listings.'
...
export function buildListingsPayload(csvRaw, label = 'listings.csv', opts = {}) {
  ...
  const hot_listings_rules =
    dataType === 'sales'
      ? 'Top 25 listing asks below transacted sale average for the same building + bedroom count (sales transactions CSV), listed in the last 30 days. Area filter applies.'
