...
const HOT_LISTING_RENT_KEY = { Studio: 'studio', '1': '1br', '2': '2br', '3': '3br', '4+': '4br' };

/**
 * Hot Listings: ≤30d listings where asking is below **transacted** rental average for that bed type
 * (same numbers as rental CSV merge — studio / 1br / 2br / 3br; 4+ only if `4br` present in txn map).
 *
 * @param {Array<{ price: number, bedKey: string, listedDate: Date|null, community: string, building: string, link: string|null }>} rows — area-filtered, valid price
 * @param {Date} thirtyDaysAgo
 * @param {boolean} dateColPresent
 * @param {{ studio?: number|null, '1br'?: number|null, '2br'?: number|null, '3br'?: number|null, '4br'?: number|null }} [txnAvgByBeds] — from rental transactions (annualised AED)
 * @returns {{ hot_listings: object[], hot_listings_note: string|null }}
 */
export function computeHotListings(rows, thirtyDaysAgo, dateColPresent, txnAvgByBeds = {}) {
  if (!dateColPresent) {
    return {
      hot_listings: [],
  ...
  const hasAnyTxn = Object.values(txnAvgByBeds).some((v) => v != null && Number(v) > 0);

  /** @type {Array<{ community: string, building: string, beds: string, price_fmt: string, pct_drop: number, link: string|null, bed_label: string, market_avg_fmt: string }>} */
  const scored = [];

  for (const r of recent) {
    const rentKey = HOT_LISTING_RENT_KEY[r.bedKey];
    if (!rentKey) continue;
    const txnAvg = txnAvgByBeds[rentKey];
    if (txnAvg == null || !(Number(txnAvg) > 0)) continue;
    const t = Number(txnAvg);
  ...
  const hot_listings = scored.slice(0, 25);
  ...
  } else if (recent.length > 0 && !hasAnyTxn) {
    hot_listings_note =
      'Hot Listings need transacted rental averages by bedroom (rental CSV). Ensure PROPERTY_RENTAL_CSV_URL is set and rental merge succeeded.';
  } else if (recent.length > 0 && hot_listings.length === 0 && hasAnyTxn) {
    hot_listings_note =
      'No listings in the last 30 days are below the transacted rental average for their bedroom count (or bedroom type has no transaction benchmark, e.g. 4+).';
  }

  return { hot_listings, hot_listings_note };
}
...
export function buildListingsPayload(csvRaw, label = 'listings.csv', opts = {}) {
  ...
  /** @type {Array<{ price: number, bedKey: string, listedDate: Date|null, community: string, building: string, link: string|null }>} */
  const listingRows = [];
  ...
  try {
    forEachCsvObject(
      ...
      if (dateCol) {
        ...
        if (listedDate) {
          if (listedDate >= sevenDaysAgo) new_this_week++;
          else if (listedDate >= fourteenDaysAgo) new_prev_7_days++;
        }
      }
  ...
  const { hot_listings, hot_listings_note } = computeHotListings(
    listingRows,
    thirtyDaysAgo,
    !!dateCol,
    txnRentRef,
  ...
  const hot_listings_rules =
    'Top 25 listing asks below transacted rental average for that bedroom count (rental CSV), listed in the last 30 days. Area filter applies. % = how far below that transacted average.';
