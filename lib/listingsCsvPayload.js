...
export function buildListingsPayload(csvRaw, label = 'listings.csv', opts = {}) {
  ...
  // Apply area filter — matches same community column used by sales/rental area selectors
  const filterArea = (opts.filterArea || '').trim();
  if (filterArea && communityCol) {
    rows = rows.filter(row => {
      const comm = getVal(row, communityCol);
      return comm.toLowerCase() === filterArea.toLowerCase();
    });
  }
  ...
  // #region agent log
  if (typeof fetch !== 'undefined') fetch('http://127.0.0.1:7603/ingest/99cc14af-5ec3-4b0c-b7f2-77017c17c844',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'69d0ba'},body:JSON.stringify({sessionId:'69d0ba',location:'listingsCsvPayload.js:return',message:'listings built after filter',data:{filterArea,total,rowsAfterFilter:rows.length,listings_top_mode,top_communities_len:top_communities.length,top_buildings_len:top_buildings.length},timestamp:Date.now()})}).catch(()=>{});
  // #endregion
