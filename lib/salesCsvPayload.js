...
export function buildPayloadFromCsvText(csvRaw, label = 'uploaded.csv', options = {}) {
  ...
  for (const r of records) {
    ...
    const entryName = distName || bldName; // 2-level: no district, use building as entry
    ...
    cur.count++;
    // Add tower as sub-building when a distinct district level exists
    if (distName && bldName && normalizeCommunityKey(bldName) !== key) {
      cur.subBuildingSet.add(bldName);
    }
  ...
  // Building filter — matches the normalized building level.
  // normBuilding is correctly assigned regardless of which CSV column held the
  // tower name (see normalizeLocationHierarchy in communityMatch.js).
