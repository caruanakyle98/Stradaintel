...
function buildFromSalesRecords({ records, weekStart, weekEnd, prevStart, prevEnd, csvPath, filterArea }) {
  ...
  // #region agent log
  fetch('http://127.0.0.1:7603/ingest/99cc14af-5ec3-4b0c-b7f2-77017c17c844',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'69d0ba'},body:JSON.stringify({sessionId:'69d0ba',runId:'pre-fix',hypothesisId:'H4',location:'lib/salesCsvPayload.js:buildFromSalesRecords',message:'Computed rolling 7-day sales windows',data:{csvPath,filterArea:filterArea||null,weekStart:weekStart.toISOString(),weekEnd:weekEnd.toISOString(),prevStart:prevStart.toISOString(),prevEnd:prevEnd.toISOString(),weekCount,prevCount,recordsCount:records.length},timestamp:Date.now()})}).catch(()=>{});
  // #endregion
...
export function buildPayloadFromCsvText(csvRaw, label = 'uploaded.csv', options = {}) {
  ...
  const filteredRecords =
    areaFilter && areaFilter !== '__all__'
      ...
      : records;
  // #region agent log
  fetch('http://127.0.0.1:7603/ingest/99cc14af-5ec3-4b0c-b7f2-77017c17c844',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'69d0ba'},body:JSON.stringify({sessionId:'69d0ba',runId:'pre-fix',hypothesisId:'H3',location:'lib/salesCsvPayload.js:buildPayloadFromCsvText',message:'Parsed sales records and applied area filter',data:{label,totalParsed:records.length,filteredCount:filteredRecords.length,areaFilter:areaFilter||null,latestParsedDate:records.length?records.reduce((m,r)=>(r.evidenceDate>m?r.evidenceDate:m),records[0].evidenceDate).toISOString():null},timestamp:Date.now()})}).catch(()=>{});
  // #endregion
