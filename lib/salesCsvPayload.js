  ...
  try {
    fetch('http://127.0.0.1:7603/ingest/99cc14af-5ec3-4b0c-b7f2-77017c17c844', {
      ...
      headers: {
        ...
        'X-Debug-Session-Id': '13de73',
      },
      body: JSON.stringify({
        sessionId: '13de73',
        runId: 'psf-debug',
        hypothesisId: 'A',
        location: 'salesCsvPayload.js:buildCharts30d',
        message: 'charts30d psf stats',
        data: {
          recordsTotal: records.length,
          psfDaysWithData: psf_daily_avg.filter(p => p.value != null).length,
          psfWeeklyPoints: psf_weekly.length,
          wow_psf_pct: wow_psf,
        },
        timestamp: Date.now(),
      }),
 ...
 * @param {{ area?: string }} [options] - if area set, keep only rows where All Developments / Area matches (exact)
...
export function buildPayloadFromCsvText(csvRaw, label = 'uploaded.csv', options = {}) {
  ...
  const columnMap = {
    ...
    /** Master area (filter) — All Developments first; else single-location exports */
    area: pickColumn(headers, ['All Developments', 'all developments', 'Area', 'Project Name', 'Community']),
    /** Tower / building line — .1 column (regex fallback for header variants). */
    subCommunityTower: pickSubCommunityBuilding1(headers),
    /** Master export: sub-area + building (Ocean Heights, Princess Tower, …) — use when .1 empty */
    communityBuilding: pickColumn(headers, [
      'Community/Building',
      'community/building',
    ]),
    /** Second sub column (tower) when .1 missing */
  ...
  // Some exports duplicate PSF as "price (aed/sq ft)" and "price (aed/sq. ft)".
  // Find a second header with the same normalized key so we can fall back when the first is empty.
  let psfAltHeader = null;
  if (columnMap.psfAed) {
    const normMain = normalizeKey(columnMap.psfAed);
    psfAltHeader =
      headers.find(
        (h) => h !== columnMap.psfAed && normalizeKey(h) === normMain,
      ) || null;
  }
  ...
  try {
    fetch('http://127.0.0.1:7603/ingest/99cc14af-5ec3-4b0c-b7f2-77017c17c844', {
      ...
      body: JSON.stringify({
        ...
        runId: 'psf-debug',
        hypothesisId: 'B',
        location: 'salesCsvPayload.js:buildPayloadFromCsvText',
        message: 'psf column detection',
        data: {
          label,
          psfHeaderPrimary: columnMap.psfAed || null,
          psfHeaderAlt: psfAltHeader,
          hasPsfHeader: !!columnMap.psfAed,
  ...
  const records = rows.map(r => {
    ...
    // Drill-down label: tower > community/building (master) so area filter still buckets by building
    ...
    return {
      ...
      /** Community/Building raw — drill fallback when .1 empty but building ≠ area */
  ...
  const areaOptions = [...new Set(records.map(r => r.area).filter(Boolean))].sort((a, b) =>
  ...
  const filteredRecords =
    areaFilter && areaFilter !== '__all__'
      ? records.filter(r => r.area === areaFilter)
      : records;
