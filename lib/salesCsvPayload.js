...
/**
 * Last N Dubai days: daily counts + PSF; 7d MA; weekly sum volume + median PSF + IQR.
 */
function buildCharts30d(records, days = 30) {
  ...
  // Use an N-day window ending **yesterday** (exclude today's partial data).
...
function buildMonthly(records) {
  // Dubai is UTC+4. Compute all boundaries as UTC timestamps anchored to Dubai midnight
  // to avoid labels flipping month when a server runs in a different timezone.
  const DUBAI_OFFSET_MS = 4 * 60 * 60 * 1000;

  // Get current Dubai date components from a local-timezone Date that mirrors Dubai wall time
  const dubaiNow  = new Date(new Date().toLocaleString('en-US', { timeZone: 'Asia/Dubai' }));
  const dubaiYear  = dubaiNow.getFullYear();
  const dubaiMonth = dubaiNow.getMonth(); // 0-indexed
  const dubaiDay   = dubaiNow.getDate();

  // Boundary timestamps (UTC) — Dubai midnight = UTC 20:00 prior calendar day
  ...
  const prevStart = new Date(Date.UTC(dubaiYear, dubaiMonth - 1, 1)       - DUBAI_OFFSET_MS); // Date.UTC handles month=0 rollover
  const prevEnd   = new Date(curStart.getTime() - 1);

  // Labels derived from date components directly — never from formatting a boundary timestamp
  // (formatting prevEnd with Dubai TZ could land in the next month if prevEnd is ~midnight UTC)
  ...
  const inMonth = (r, s, e) => r.evidenceDate && r.evidenceDate >= s && r.evidenceDate <= e;
  ...
  const psfAverage = recs => {
    const vals = recs.map(r => r.psfAed).filter(v => v != null && Number.isFinite(v));
    if (!vals.length) return null;
    return Math.round(vals.reduce((a, b) => a + b, 0) / vals.length);
  };

  const avgByBeds = recs => {
    const BED_KEYS = ['Studio', '1', '2', '3', '4+'];
    const out = {};
    for (const bk of BED_KEYS) {
      const prices = recs.filter(r => r.bedKey === bk && r.priceAed > 0).map(r => r.priceAed);
      if (prices.length) {
        const avg = Math.round(prices.reduce((a, b) => a + b, 0) / prices.length);
        out[bk] = { avg, count: prices.length, avg_fmt: `AED ${fmtCompact(avg)}` };
      }
    }
    return out;
  };
  ...
  const curBeds  = avgByBeds(curRecs);
  const prevBeds = avgByBeds(prevRecs);

  const momBeds = {};
  for (const bk of ['Studio', '1', '2', '3', '4+']) {
    if (curBeds[bk] && prevBeds[bk])
      momBeds[bk] = momPct(curBeds[bk].avg, prevBeds[bk].avg);
  }
  ...
  return {
    ...
    cur_avg_by_beds:     curBeds,
    prev_avg_by_beds:    prevBeds,
    mom_avg_by_beds_pct: momBeds,
    cur_days_elapsed:    dubaiDay,
  };
}

function buildFromSalesRecords({ records, weekStart, weekEnd, prevStart, prevEnd, days, csvPath, filterArea, filterCommunity, filterBuilding }) {
  ...
  const chgPeriodLabel = `vs prior ${windowDays} days`;
  const areaSuffix = filterBuilding ? ` · Building: ${filterBuilding}` : filterArea ? ` · Area: ${filterArea}` : '';
  const period = `Last ${windowDays} days (Dubai) — ${thisWeekLabel}${areaSuffix}`;
  ...
  // ROI yield sale references: up to last 90 days, or all available filtered rows if none in-window.
  const yieldLookbackDays = 90;
  const nowDubai = new Date(new Date().toLocaleString('en-US', { timeZone: 'Asia/Dubai' }));
  const yieldCutoff = new Date(nowDubai.getTime() - yieldLookbackDays * 24 * 60 * 60 * 1000);
  const saleYieldWindowRows = records.filter(
    (r) => r.evidenceDate && r.evidenceDate >= yieldCutoff && Number(r.priceAed) > 0,
  );
  const saleYieldRows = saleYieldWindowRows.length > 0 ? saleYieldWindowRows : records;
  const avgDealForYield = (type) => avg(saleYieldRows.filter((r) => r.unitType === type), (r) => r.priceAed);
  const saleYieldApt = avgDealForYield('apt');
  const saleYieldVilla = avgDealForYield('villa');
  const saleYieldTownhouse = avgDealForYield('townhouse');
  const yieldSalesRows = records
    .filter((r) => r.evidenceDate && Number(r.priceAed) > 0 && (r.unitType === 'apt' || r.unitType === 'villa' || r.unitType === 'townhouse'))
    .map((r) => ({
      ts: r.evidenceDate.getTime(),
      unitType: r.unitType,
  ...
  // Use normBuilding for drill-down grouping — correctly resolved regardless of column order
  const drillKey = r => r.normBuilding || r.normDistrict || null;
  const topKey = r => (drillDown ? drillKey(r) : r.area || 'Unknown');

  const byBucket = new Map();
  const byBucketPrev = new Map();

  function addWeekRow(r) {
    const key = drillDown ? drillKey(r) : r.area || 'Unknown';
    if (!key) return;
  ...
  const charts_30d = buildCharts30d(records, windowDays);
  ...
  for (const r of records) {
    ...
    if (!r.bedKey || r.bedKey === 'Other') continue;
    ...
    const commPrefix = filterArea
      ...
      : normalizeCommunityKey(r.area || '');
    if (commPrefix) {
      const cKey = `${commPrefix}|${r.bedKey}|${uk}`;
      const cCur = txnByCommunityBedAgg.get(cKey) || { sum: 0, count: 0 };
      cCur.sum += Number(r.priceAed) || 0;
      cCur.count += 1;
  ...
  const sale_txn_by_community_bed = {};
  for (const [key, agg] of txnByCommunityBedAgg.entries()) {
    if (agg.count < hotMinTxn) continue;
    const avg = agg.sum / agg.count;
    if (!(avg > 0)) continue;
    sale_txn_by_community_bed[key] = { avg: Math.round(avg), n: agg.count };
  }
  ...
  const recent_sales_transactions = [...records]
    ...
    .map((r) => ({
      ...
      area: r.area || '—',
      location: r.normBuilding || r.normDistrict || '—',
  ...
  return {
    ...
    weekly: {
      sale_volume: { value: String(weekCount), chg_wow: pctChange(weekCount, prevCount), chg_period_label: chgPeriodLabel, chg_yoy: 'N/A', trend: trendFrom(weekCount, prevCount), period, source },
      sale_value_aed: { value: `AED ${fmtCompact(weekValue)}`, chg_wow: pctChange(weekValue, prevValue), chg_period_label: chgPeriodLabel, chg_yoy: 'N/A', trend: trendFrom(weekValue, prevValue), period, source },
    ...
    prices: {
      ...
      // Yield gauge references (do not alter headline weekly pricing cards).
      apt_avg_aed_yield_ref_num: saleYieldApt ?? null,
      villa_avg_aed_yield_ref_num: saleYieldVilla ?? null,
      townhouse_avg_aed_yield_ref_num: saleYieldTownhouse ?? null,
      yield_ref_lookback_days: String(yieldLookbackDays),
    ...
    top_areas_empty_hint:
      drillDown && topAreas.length === 0
        ? `No building-level rows in the last ${windowDays} days: fill sub community / building.1, or use Community/Building when it differs from the area name.`
        : null,
    ...
    owner_briefing: filterArea
      ? `Area filter “${filterArea}”. Rolling ${windowDays} days (${thisWeekLabel}, Dubai): ${weekCount.toLocaleString('en-US')} transactions worth AED ${fmtCompact(weekValue)}. Prior ${windowDays} days: ${prevCount.toLocaleString('en-US')} deals. Off-plan ${offplanPct == null ? 'N/A' : offplanPct + '%'}.`
      : `Rolling ${windowDays} days through today (${thisWeekLabel}, Dubai): ${weekCount.toLocaleString('en-US')} transactions in your CSV worth AED ${fmtCompact(weekValue)}. Prior ${windowDays} days (${prevLabel}): ${prevCount.toLocaleString('en-US')} deals. Off-plan share ${offplanPct == null ? 'N/A' : offplanPct + '%'} (Oqood-tagged rows).`,
    data_freshness: latestDate ? `Transactions through ${fmtDate(latestDate)}` : period,
    sources_used: [source],
    sale_txn_avg_by_beds,
    monthly,
    sale_txn_by_building_bed,
    sale_txn_by_community_bed,
    sale_hot_listings_lookback_days: String(hotLookbackDays),
    sale_hot_listings_min_txn: String(hotMinTxn),
    // ── Property type activity — area level only (not community/building filtered) ──────────────
    // Populated here with sales data; rental volumes are merged in by rentalCsvPayload.
    property_type_activity: (() => {
      if (filterArea || filterCommunity || filterBuilding) return null;
      const TYPES = ['apt', 'villa', 'townhouse'];
      const LABELS = { apt: 'Apartment', villa: 'Villa', townhouse: 'Townhouse' };
      const curVol  = Object.fromEntries(TYPES.map(t => [t, week.filter(r => r.unitType === t).length]));
      const prevVol = Object.fromEntries(TYPES.map(t => [t, prev.filter(r => r.unitType === t).length]));
      const saleTotal = TYPES.reduce((s, t) => s + curVol[t], 0);
      const fmtWoW = (cur, prv) => prv > 0 ? ((cur - prv) / prv * 100).toFixed(1) + '%' : null;
      const entries = Object.fromEntries(TYPES.map(t => [t, {
        label:      LABELS[t],
        sale_vol:   curVol[t],
        sale_pct:   saleTotal > 0 ? Math.round((curVol[t] / saleTotal) * 100) : 0,
        sale_wow:   fmtWoW(curVol[t], prevVol[t]),
        sale_trend: prevVol[t] > 0 ? (curVol[t] >= prevVol[t] ? 'up' : 'down') : null,
        // rental fields merged later by rentalCsvPayload
        rent_vol: null, rent_pct: null, rent_wow: null, rent_trend: null,
      }]));
      const hottestSale = saleTotal > 0
        ? TYPES.reduce((a, b) => curVol[a] >= curVol[b] ? a : b)
        : null;
      return { period, entries, hottest_sale: hottestSale, hottest_rent: null };
    })(),
    _stats_for_ai: {
      window: `rolling_${windowDays}d_dubai_vs_prior_${windowDays}d`,
    ...
    // Internal handoff so rental merge can align ROI yield windows to the same effective date range.
    _yield_sales_rows: yieldSalesRows,
  };
}

/**
 * Rolling windows in Asia/Dubai (calendar days, inclusive).
 * @param {any} _records - unused, kept for backwards compat
 * @param {{ days?: number }} [opts] - window size in days (e.g. 7, 30, or 90). Default 7.
 * Current: today 00:00 through today 23:59 + (days-1) prior days.
 * Previous: the same number of calendar days immediately before that (for period-over-period %).
 */
export function deriveAnalysisWindows(_records, opts = {}) {
  const days = Math.max(1, Math.min(90, parseInt(opts.days, 10) || 7));
  const now = new Date(new Date().toLocaleString('en-US', { timeZone: 'Asia/Dubai' }));
  const weekEnd = new Date(now);
  weekEnd.setHours(23, 59, 59, 999);
  const weekStart = new Date(now);
  weekStart.setDate(now.getDate() - (days - 1));
  ...
  return { weekStart, weekEnd, prevStart, prevEnd, days };
}

/**
 * @param {string} csvRaw
 * @param {string} label - shown in sources (e.g. file name)
 * @param {{ area?: string, building?: string, days?: number, communityAliasMap?: Map<string, string>|null }} [options]
...
export function buildPayloadFromCsvText(csvRaw, label = 'uploaded.csv', options = {}) {
  ...
  forEachCsvObject(
    ...
    (r) => {
      ...
      const cbVal = columnMap.communityBuilding
        ...
        : '';
      // col F — specific tower (Sub Community / Building.1)
      let towerVal = columnMap.subCommunityTower
        ? String(getWithAliases(r, [columnMap.subCommunityTower])).trim()
        : '';
      // col E — parent building group (Sub Community / Building) — always capture independently
      const parentVal = columnMap.subCommunityAlt
        ? String(getWithAliases(r, [columnMap.subCommunityAlt])).trim()
        : '';
      if (!towerVal) towerVal = parentVal; // fallback when col F is empty (3-level data)
      ...
      // parentBuilding: col E value, only meaningful when col F also has a distinct value (true 4-level)
      const parentBuilding = (parentVal && towerVal && towerVal !== parentVal) ? parentVal : null;
  ...
  records = normalizeLocationHierarchy(records, 'drillCommunityBuilding', 'subCommunity', 'area');

  const areaOptions = [...areaSet].sort((a, b) =>
    a.localeCompare(b, undefined, { sensitivity: 'base' }),
  );

  // Building options — keyed by normDistrict (development/community level).
  // For 2-level areas (normDistrict = null) keyed by normBuilding instead.
  // sub_buildings tracks normBuilding values within each district entry.
  // Key includes area to distinguish same-named buildings in different areas.
  const buildingMap = new Map();
  for (const r of records) {
    const distName   = (r.normDistrict   || '').trim();
    const parentName = (r.parentBuilding || '').trim();
    const bldName    = (r.normBuilding   || '').trim();
    const entryName  = distName || parentName || bldName; // 2-level: no district, use building as entry
    if (!entryName) continue;
    const area = (r.area || '').trim();
    const key = normalizeCommunityKey(entryName) + '|' + area;
    const cur = buildingMap.get(key) || { name: entryName, area, count: 0, subBuildingSet: new Set() };
    cur.count++;
    // 4-level: sub_buildings = parent building group (e.g. "Beach Mansion") so user can filter
    //          to all towers in that group without selecting each tower individually.
    // 3-level: sub_buildings = specific building name (existing behaviour).
    const subEntry = parentName || bldName;
    if (distName && subEntry && normalizeCommunityKey(subEntry) !== key.split('|')[0]) {
      cur.subBuildingSet.add(subEntry);
    }
    buildingMap.set(key, cur);
  }
  const buildingOptions = [...buildingMap.values()]
    .filter(b => b.count >= 1)
    .sort((a, b) => b.count - a.count)
    .map(({ subBuildingSet, ...rest }) => ({
      ...rest,
      sub_buildings: [...subBuildingSet].sort(),
    }));

  const areaFilter      = (options.area      || '').trim();
  const communityFilter = (options.community || '').trim();
  const buildingFilter  = (options.building  || '').trim();
  ...
  // For 2-level areas (normDistrict = null) falls back to normBuilding so the
  // community dropdown entry (which is the building name) still filters correctly.
  if (communityFilter && communityFilter !== '__all__') {
    filteredRecords = filteredRecords.filter((r) =>
      communitiesMatch(r.normDistrict,   communityFilter, null) ||
      communitiesMatch(r.normBuilding,   communityFilter, null) ||
      communitiesMatch(r.parentBuilding, communityFilter, null)
    );
  }
  ...
  if (buildingFilter && buildingFilter !== '__all__') {
    filteredRecords = filteredRecords.filter((r) => {
      if (r.parentBuilding && communitiesMatch(r.parentBuilding, buildingFilter, null)) return true;
      if (r.normBuilding   && communitiesMatch(r.normBuilding,   buildingFilter, null)) return true;
      if (r.normDistrict   && communitiesMatch(r.normDistrict,   buildingFilter, null)) return true;
      return false;
    });
  }
  ...
  if (buildingFilter && buildingFilter !== '__all__' && !filteredRecords.length) {
    return {
      ok: false,
      status: 400,
      body: {
        ok: false,
        error: `No transactions for building “${buildingFilter}”. Choose another building or clear the filter.`,
        area_options: areaOptions,
        building_options: buildingOptions,
      },
    };
  ...
  const windows = deriveAnalysisWindows(filteredRecords, { days: windowDays });
  const payload = buildFromSalesRecords({
    records: filteredRecords,
    ...windows,
    csvPath: label,
    filterArea:      areaFilter      && areaFilter      !== '__all__' ? areaFilter      : null,
    filterCommunity: communityFilter && communityFilter !== '__all__' ? communityFilter : null,
    filterBuilding:  buildingFilter  && buildingFilter  !== '__all__' ? buildingFilter  : null,
  });
  payload.area_options     = areaOptions;
  payload.building_options = buildingOptions;
  payload.filter_area      = areaFilter      && areaFilter      !== '__all__' ? areaFilter      : null;
  payload.filter_community = communityFilter && communityFilter !== '__all__' ? communityFilter : null;
  payload.filter_building  = buildingFilter  && buildingFilter  !== '__all__' ? buildingFilter  : null;
