...
function buildMonthly(records) {
  ...
  // Current month-to-date (Dubai timezone)
  const curStart = new Date(now.getFullYear(), now.getMonth(), 1, 0, 0, 0, 0);
  const curEnd = new Date(now); curEnd.setHours(23, 59, 59, 999);
  ...
  const curLabel  = now.toLocaleString('en-US', { month: 'long', year: 'numeric', timeZone: 'Asia/Dubai' });
  const prevLabel = prevEnd.toLocaleString('en-US', { month: 'long', year: 'numeric', timeZone: 'Asia/Dubai' });
