...
function buildFromSalesRecords({ records, weekStart, weekEnd, prevStart, prevEnd, csvPath, filterArea }) {
  ...
  /** Filtered view: only bucket by sub community / building.1 — never fall back to area (avoids repeating the selected area name). */
  ...
  for (const r of week) {
    if (drillDown && !r.subCommunity) continue;
    const key = topKey(r);
...
export function buildPayloadFromCsvText(csvRaw, label = 'uploaded.csv', options = {}) {
  ...
  const columnMap = {
    ...
    /** Drill-down when filtered — Property Monitor export uses this header */
    subCommunity: pickColumn(headers, [
  ...
  fetch('http://127.0.0.1:7603/ingest/99cc14af-5ec3-4b0c-b7f2-77017c17c844', {
    ...
    body: JSON.stringify({
      sessionId: '13de73',
      hypothesisId: 'H1',
      location: 'salesCsvPayload.js:columnMap',
      message: 'subCommunity column picked',
      data: {
        subCommunity: columnMap.subCommunity,
        area: columnMap.area,
        headerMatch: headers.some(h => String(h).toLowerCase().includes('building.1')),
      },
      timestamp: Date.now(),
    }),
  }).catch(() => {});
  ...
  const records = rows.map(r => {
    ...
    let subVal = columnMap.subCommunity ? String(getWithAliases(r, [columnMap.subCommunity])).trim() : '';
    if (columnMap.subCommunity && columnMap.area && columnMap.subCommunity === columnMap.area) subVal = '';
    if (!areaVal && subVal) {
