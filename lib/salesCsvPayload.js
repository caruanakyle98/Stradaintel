...
export function buildPayloadFromCsvText(csvRaw, label = 'uploaded.csv', options = {}) {
  ...
  // Building options — use drillCommunityBuilding (Community/Building column) as primary,
  // falling back to subCommunity only when no community/building column exists.
  // This keeps filtering at the development/community level (e.g. "The Residences at Marina Gate")
  // rather than individual tower level ("The Residences at Marina Gate 2").
  // Track sub-buildings for areas with multi-level hierarchy (e.g. Downtown Dubai → Old Town → Old Town Island).
  const buildingMap = new Map();
  for (const r of records) {
    const bName = (r.drillCommunityBuilding || r.subCommunity || '').trim();
    if (!bName) continue;
    const key = normalizeCommunityKey(bName);
    const cur = buildingMap.get(key) || { name: bName, area: r.area || '', count: 0, subBuildingSet: new Set() };
    cur.count++;
    // Track sub-buildings: Level 3 values that are distinct from Level 2
    if (r.subCommunity && r.subCommunity.trim() !== bName) {
      cur.subBuildingSet.add(r.subCommunity.trim());
    }
  ...
  // Community filter (Level 2 only — e.g. "Old Town" district)
  if (communityFilter && communityFilter !== '__all__') {
    filteredRecords = filteredRecords.filter((r) =>
      communitiesMatch(r.drillCommunityBuilding || r.subCommunity, communityFilter, null)
    );
  }

  // Building filter.
  // When communityFilter is NOT set: match Level 2 || Level 3 (drillCommunityBuilding || subCommunity).
  // When communityFilter IS set: DLD exports vary — some areas put the tower name in Level 2
  //   (drillCommunityBuilding) and the development name in Level 3 (subCommunity); others do the
  //   reverse. Check both columns so the filter works regardless of which column the area uses.
  if (buildingFilter && buildingFilter !== '__all__') {
    filteredRecords = filteredRecords.filter((r) => {
      if (communityFilter) {
        const sub   = (r.subCommunity          || '').trim();
        const drill = (r.drillCommunityBuilding || '').trim();
        return (sub   && communitiesMatch(sub,   buildingFilter, null)) ||
               (drill && communitiesMatch(drill, buildingFilter, null));
      }
      const bName = (r.drillCommunityBuilding || r.subCommunity || '').trim();
