...
- **Drill-down (when filtered):** **`sub community / building.1`** only — rows with that column empty are omitted from the ranked list (the area name is never used as a stand-in).
